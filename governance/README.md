...
- **Registry creation invariant (the creation-bypass fix):** a record newly created under a lifecycle-managed registry tree must ACTUALLY traverse its lifecycle from the initial status — the protected-path check reads the record's full commit history within the change (`checkRegistryAdditionTraversal` in `tools/governance/src/registry-lifecycle.ts`): the INTRODUCTION commit must be born at the INITIAL status (a real, non-demo ACR is created as `PROPOSED`; a real reconciliation as `STAGED`) and EVERY later change to the same new record within the same change must itself be a narrowly content-checked legal lifecycle transition. The lifecycle-transition guard only ever sees a before/after pair for MODIFICATIONS of pre-existing paths; for a creation there is no before, so without this invariant a PR could birth an already-`APPROVED` (or `IMPLEMENTED`) ACR — or an already-`DECIDED` reconciliation — as an "allowed registry addition", bypassing the entire review/approval chain (and an endpoint-only check would let a born-`PROPOSED` record be quietly tampered mid-change and still land `APPROVED`). Lawful creations are reported explicitly as `REGISTRY ADDITION …` — with the traversed legal edges named when the same change advanced the record (this is exactly what keeps the documented review-time acts flow lawful: a record born `PROPOSED` on the PR branch may be advanced to `APPROVED` by the Architect's acts commit, checked as the legal transition it must be). Demo fixtures (the record's own `demo: true` marking) are exempt from the initial-status rule — they exist to demonstrate the mechanisms end-to-end and are inert (they can never route protected-path changes or sanction real reconciliations; demo ACRs must use the reserved `ACR-9xx` range — enforced by the validate step) — though later edits to a new demo record within the same change are still checked as transitions. Unreadable history fails closed. The general channel survives: an approved ACR that explicitly enumerates the exact creation path can still authorize an out-of-band registration (e.g. re-registering an externally-approved proposal), so the invariant is a floor for ordinary PRs, not an absolute bar for explicitly dual-gate-authorized registrations.
- **Registry lifecycle rule (the circular-authorization fix):** the two registry trees are additionally lifecycle-managed (`lifecycle: "acr"` / `lifecycle: "reconciliation"`). A modification of an EXISTING registry record is waived only when it is a narrowly content-checked legal lifecycle transition of that record — same identity, a legal status edge (ACR: PROPOSED → ENDORSED/REJECTED, ENDORSED → APPROVED/REJECTED, APPROVED → IMPLEMENTED, and their monotone compositions; reconciliation: STAGED → DECIDED), the changed top-level fields EXACTLY `{status}` plus the edge's gate instruments, every instrument newly added with its authorizing role (architect review / product-owner approval / architect decision) and required verdict (`tools/governance/src/registry-lifecycle.ts`). This is what lets an ACR complete its own lifecycle — its advancement is authorized by the lifecycle rules themselves, not by another ACR (which would recurse forever). Everything else — prose amendments, `authorized_paths` edits, evidence/citation changes, instrument replacement, key removal, id changes, deletions — is an ordinary modification and stays a violation unless an approved ACR explicitly enumerates the record path (the general channel). Waived transitions are reported explicitly as `REGISTRY LIFECYCLE …` in the check-protected output, never silently. In strict mode (no base ref) unverifiable modifications AND unverifiable creations fail closed.
...
- **Born PROPOSED (creation invariant):** a new real ACR record enters the registry at `PROPOSED` and, if the same change advances it, only through the narrowly checked legal transitions — the protected-path check verifies the record's full commit-range traversal (see §6), so no record can be created mid-lifecycle (already `APPROVED`/`IMPLEMENTED`) or quietly tampered mid-change, and thereby skip the gates. Demo fixtures (`demo: true`, reserved `ACR-9xx` range) are exempt but inert.
...
- **Two states:** `STAGED` — the facts, verbatim original-event citations and evidence are complete, the Architect decision is pending, and **no waivers are active** (the ledger failures stay fully visible); `DECIDED` — the Architect has recorded the decision and the enumerated waivers activate. A new real reconciliation is staged as `STAGED` (creation invariant — a reconciliation cannot be born `DECIDED`; the decision must traverse the narrowly checked `STAGED → DECIDED` edge).
