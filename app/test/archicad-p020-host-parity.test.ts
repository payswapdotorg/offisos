/**
 * CAD-PARITY-020 (Issue #123) — Web/Electron host parity for the
 * Archicad-class parity certification: the SAME version-pinned Archicad
 * corpus (archicad-p020-corpus/1) executed through the Web Host
 * (WebSocketTransport) and the Electron Host (IpcTransport) — each driving
 * its OWN handler + bundle instance through its REAL transport — produces
 * byte-identical certification verdicts: the per-workflow
 * phase/expectation/interop/robustness results and the aggregate summary
 * converge exactly across hosts (LOCK-004; mirrors the P019 host-parity
 * discipline — the P020 acceptance criterion: the certified workflow
 * remains consistent with the P019 baseline wherever shared CAD semantics
...
import { runCertification, P020_ARCHICAD_CORPUS_BUNDLE, type CertDriver, type DriverResult, type CertificationReport } from "../src/certification/engine.js";
...
function makeHostRenderer(kind: "web" | "electron"): { renderer: Renderer; driver: CertDriver } {
  const handler = AppApiHandler.create({
    ...
    entityId: `p020-parity-${kind}`,
    format: "offisos-reference",
    formatVersion: "1",
    createdBy: "p020-parity",
  });
 ...
 *  content-addressed hex normalized (the P016/P017/P018/P019 parity discipline). */
...
test("CAD-PARITY-020: the Archicad-class parity certification converges byte-identically across the Web and Electron hosts", { skip: skipIfc }, async () => {
  ...
  const webRun = await runCertification(web.driver, {
    ...
    basisNote: "The host-parity certification basis: the reference adapter + the pinned IfcOpenShell interop adapter through the Web host's REAL WebSocketTransport (the Archicad-class corpus through the shared prompt-engine command registry).",
  });
  const electronRun = await runCertification(electron.driver, {
    driverKind: "ipc-transport",
    corpus: P020_ARCHICAD_CORPUS_BUNDLE,
    basisNote: "The host-parity certification basis: the reference adapter + the pinned IfcOpenShell interop adapter through the Electron host's REAL IpcTransport (the Archicad-class corpus through the shared prompt-engine command registry).",
  });
