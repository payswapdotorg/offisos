/**
 * COMPAT-CAD-008 (Issue #5) — deterministic implementation coverage for the
 * ARRAY/materialization surface (CAD-BENCH-RW-001 DEF-015) over the VERIFIED
 * COMPAT-CAD-007 selection/edit foundation.
 *
 * LIFECYCLE: COMPAT-CAD-007 is VERIFIED at physical merge 9232c90 and
 * COMPAT-CAD-008 is ASSIGNED to z-ai-implementation-agent (governance record
 * governance/work-items/COMPAT-CAD-008.json; roadmap
 * docs/cad/autocad-parity-roadmap.md §12). This suite is the implementation
 * evidence for the frozen ARRAY scope on branch work/compat-cad-008-array,
...
// ---------------------------------------------------------------------------
// COMPAT-CAD-008 DEC-001 remediation fixtures (Issue #5):
//   R1 — ARRAY/member provenance: deterministic, links to op + source + index.
//   R2 — source-deletion cascade: no orphaned members; exact undo/redo.
//   R3 — individual member deletion: normal (no upward cascade).
//   R4 — path mode typed `unsupported` at the canonical semantic boundary.
//   R5 — provenance byte-identity on repeated execution.
//   R6 — polar provenance: deterministic, links to op + source + index.
// ---------------------------------------------------------------------------

/** Read the arrayProvenance from an element's props (the canonical entity
 *  model surface — not a separate authority). */
function provenanceOf(el: ElementRow): Record<string, unknown> | null {
  const ap = el.props.arrayProvenance;
  if (ap === null || typeof ap !== "object") return null;
  return ap as Record<string, unknown>;
}

test("remediation R1 — ARRAY rectangular members carry deterministic provenance linking to op, source and memberIndex", async () => {
  const h = await seeded();
  val(
    await h.handle(cmd("entity.modify", { op: "array", mode: "rectangular", ids: ["el-000001"], rows: 2, columns: 3, rowSpacing: 50, columnSpacing: 200 })),
  );
  const after = await stateOf(h);
  // The source occurrence (0,0) is NOT a member and carries NO provenance.
  const src = after.elements.find((el) => el.id === "el-000001")!;
  assert.equal(provenanceOf(src), null, "the source occurrence is not tagged (it is not a member)");

  // Row-major member order and the expected (memberIndex, row, column):
  // (r0,c1)=idx1, (r0,c2)=idx2, (r1,c0)=idx3, (r1,c1)=idx4, (r1,c2)=idx5.
  ...
  for (const [id, memberIndex, row, column] of expected) {
    const ap = provenanceOf(after.elements.find((el) => el.id === id)!);
    assert.ok(ap !== null, `${id} carries arrayProvenance`);
    assert.equal(ap!.sourceId, "el-000001", `${id} provenance links to the source occurrence`);
    assert.equal(ap!.mode, "rectangular", `${id} provenance mode`);
    assert.equal(ap!.memberIndex, memberIndex, `${id} deterministic row-major memberIndex`);
    assert.equal(ap!.row, row, `${id} row`);
    assert.equal(ap!.column, column, `${id} column`);
    assert.ok(typeof ap!.opId === "string" && (ap!.opId as string).startsWith("array:rectangular:"), `${id} opId fingerprint`);
    opIds.add(ap!.opId as string);
  }
  assert.equal(opIds.size, 1, "all members share one deterministic opId (same ARRAY operation)");
});

test("remediation R2 — source deletion cascades to ARRAY members (no orphans); exact undo/redo", async () => {
  const h = await seeded();
  val(
    await h.handle(cmd("entity.modify", { op: "array", mode: "rectangular", ids: ["el-000001"], rows: 2, columns: 3, rowSpacing: 50, columnSpacing: 200 })),
  );
  const postArray = await stateOf(h);
  assert.equal(postArray.elements.length, 8, "3 sources + 5 members");
  const memberIds = ["el-000004", "el-000005", "el-000006", "el-000007", "el-000008"];

  // Delete the SOURCE (el-000001). The cascade must remove the source AND
  // all 5 ARRAY-owned members in ONE atomic revision — no orphans.
  const delResult = val<{ applied: boolean; summary: string }>(
    await h.handle(cmd("drafting.delete", { ids: ["el-000001"] })),
  );
  assert.equal(delResult.applied, true);
  assert.ok(delResult.summary.includes("cascade-deleted"), `summary mentions cascade: ${delResult.summary}`);
  const deleted = await stateOf(h);
  assert.equal(deleted.revisions, postArray.revisions + 1, "ONE atomic revision for source + cascade");
  assert.ok(!deleted.elements.some((el) => el.id === "el-000001"), "source removed");
  for (const id of memberIds) {
    assert.ok(!deleted.elements.some((el) => el.id === id), `member ${id} cascade-deleted (no orphan)`);
  }
  assert.equal(deleted.elements.length, 2, "only the two un-arrayed sources survive (el-000002, el-000003)");

  // UNDO restores the EXACT post-ARRAY state (source + all 5 members).
  val(await h.handle(cmd("document.undo", {})));
  const undone = await stateOf(h);
  const byId = (els: readonly ElementRow[]): ElementRow[] => [...els].sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
  assert.deepEqual(byId(undone.elements), byId(postArray.elements), "exact post-ARRAY restoration (source + all members)");

  // REDO re-erases the source AND all members.
  val(await h.handle(cmd("document.redo", {})));
  const redone = await stateOf(h);
  assert.equal(redone.elements.length, 2, "redo re-erases source + members");
  assert.ok(!redone.elements.some((el) => el.id === "el-000001"), "source re-erased");
  for (const id of memberIds) {
    assert.ok(!redone.elements.some((el) => el.id === id), `member ${id} re-erased by redo`);
  }
});

test("remediation R3 — individual member deletion is normal (no upward cascade to source or siblings)", async () => {
  const h = await seeded();
  val(
    await h.handle(cmd("entity.modify", { op: "array", mode: "rectangular", ids: ["el-000001"], rows: 2, columns: 3, rowSpacing: 50, columnSpacing: 200 })),
  );
  const postArray = await stateOf(h);
  // Delete ONE member (el-000006). No cascade: the source and the other 4
  // members survive. Only el-000006 is removed in one revision.
  val(await h.handle(cmd("drafting.delete", { ids: ["el-000006"] })));
  const after = await stateOf(h);
  assert.equal(after.revisions, postArray.revisions + 1, "one revision");
  assert.ok(after.elements.some((el) => el.id === "el-000001"), "source survives (no upward cascade)");
  assert.ok(!after.elements.some((el) => el.id === "el-000006"), "the one member removed");
  for (const id of ["el-000004", "el-000005", "el-000007", "el-000008"]) {
    assert.ok(after.elements.some((el) => el.id === id), `sibling member ${id} survives (no sibling cascade)`);
  }
  assert.equal(after.elements.length, postArray.elements.length - 1, "exactly one element removed");
});

test("remediation R4 — path mode is typed `unsupported` at the canonical semantic boundary (entity.modify / opArray)", async () => {
  const h = await seeded();
  const before = await fullStateOf(h);
  // The typed decline is at the op layer (entity.modify → opArray), not
  // merely the prompt-plan layer. The error code is `unsupported`
  // (distinct from `bad_input` for invalid parameters).
  const code = errCode(await h.handle(cmd("entity.modify", { op: "array", mode: "path", ids: ["el-000001"] })));
  assert.equal(code, "unsupported", "path mode is typed unsupported at the canonical semantic boundary");
  // No mutation: the canonical state is byte-identical to before.
  const after = await fullStateOf(h);
  assert.equal(after, before, "no canonical mutation on typed-unsupported decline");
  // Degenerate path inputs also decline typed (not crash, not fabricate).
  assert.equal(errCode(await h.handle(cmd("entity.modify", { op: "array", mode: "path", ids: [] }))), "bad_input", "empty source ids is bad_input (before the path decline)");
});

test("remediation R5 — ARRAY provenance is byte-identical on repeated execution", async () => {
  const run = async (): Promise<string> => {
    ...
    const s = await stateOf(h);
    // Project only the provenance fields (deterministic; element ids are
    // minted deterministically too — same history position ⇒ same ids).
    return JSON.stringify(
      s.elements
        .filter((el) => el.props.arrayProvenance !== undefined && el.props.arrayProvenance !== null)
        .map((el) => ({ id: el.id, ap: el.props.arrayProvenance })),
    );
  };
  const a = await run();
  const b = await run();
  assert.equal(a, b, "byte-identical provenance (opId, sourceId, memberIndex, row, column) on repeated execution");
});

test("remediation R6 — ARRAY polar members carry deterministic provenance linking to op, source and memberIndex", async () => {
  const h = await seeded();
  val(
    await h.handle(cmd("entity.modify", { op: "array", mode: "polar", ids: ["el-000001"], center: { x: 0, y: 0 }, items: 4, angleSpan: Math.PI * 2 })),
  );
  const after = await stateOf(h);
  // Polar members: el-000004 (idx1), el-000005 (idx2), el-000006 (idx3).
  ...
  const opIds = new Set<string>();
  for (const [id, memberIndex] of expected) {
    const ap = provenanceOf(after.elements.find((el) => el.id === id)!);
    assert.ok(ap !== null, `${id} carries arrayProvenance`);
    assert.equal(ap!.sourceId, "el-000001", `${id} polar provenance links to the source occurrence`);
    assert.equal(ap!.mode, "polar", `${id} provenance mode`);
    assert.equal(ap!.memberIndex, memberIndex, `${id} deterministic polar memberIndex`);
    assert.ok(typeof ap!.opId === "string" && (ap!.opId as string).startsWith("array:polar:"), `${id} opId fingerprint`);
    assert.equal(ap!.row, undefined, `${id} polar members carry no row`);
    assert.equal(ap!.column, undefined, `${id} polar members carry no column`);
    opIds.add(ap!.opId as string);
  }
  assert.equal(opIds.size, 1, "all polar members share one deterministic opId");
  // The source is not tagged.
  assert.equal(provenanceOf(after.elements.find((el) => el.id === "el-000001")!), null, "polar source is not tagged");
});
