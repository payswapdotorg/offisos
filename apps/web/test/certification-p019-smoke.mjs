...
// expectation is assessed with its honest outcome classification, and the
// cross-cutting arms (save→open→save content round-trip, undo/redo
// atomicity, replay determinism, the DRY DXF/IFC/toolsets interop probes,
// the sheet export surfaces) run against the REAL server. The semantic
// verdict surfaces must match the pinned fixture byte-identically (the
// same normalized verdicts the app-suite certification test pins on the
...
const FIXTURE_PATH = join(REPO_ROOT, "app", "test", "fixtures", "cad-parity-019-certification.json");
const WRITE_FIXTURE = process.argv.includes("--write-fixture");

const BASE = process.env.OFFISOS_WEB_URL ?? "http://localhost:3100";

const { runCertification, pinnedProjection, reportSha256 } = await import(join(REPO_ROOT, "app", "src", "certification", "engine.ts"));

const step = (name) => console.log(`CERTIFICATION P019 SMOKE: ${name}`);
...
step(`corpus run against ${BASE} (the version-pinned AutoCAD professional workflow corpus)`);
const { report } = await runCertification(driver, {
  driverKind: "web-http",
  basisNote: "The web-smoke certification basis: the REAL Next.js web app over HTTP (the reference adapter + the pinned IfcOpenShell interop adapter bound at the wiring point), driven through the shared prompt-engine command registry.",
});
...
console.log(`CERTIFICATION P019 SMOKE: pinned verdict-projection sha256 ${sha}`);
...
if (WRITE_FIXTURE) {
  ...
  console.log("CERTIFICATION P019 SMOKE: fixture written (regenerate deliberately with --write-fixture)");
} else if (!existsSync(FIXTURE_PATH)) {
  console.log("CERTIFICATION P019 SMOKE: fixture absent — created");
  mkdirSync(join(REPO_ROOT, "app", "test", "fixtures"), { recursive: true });
  writeFileSync(FIXTURE_PATH, `${JSON.stringify({ reportSha256: sha, normalized: JSON.parse(normalized) }, null, 2)}\n`);
} else {
