/**
 * INFRA-002 (fork Issue #2) — the REAL-PostgreSQL store proof (the CI web
 ...
 * typed document_corrupt — reject, never guess; the prior Architect review's
 * mandatory fork acceptance constraints).
 *
...
async function main() {
  ...
  // --- 5. The LOCK-007 stored-corruption proofs (the prior Architect review's
  // mandatory fork acceptance constraints): both corruption cases are injected with direct
