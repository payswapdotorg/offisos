// COMPAT-CAD-002 / Issue #39: reproducible 3D/BIM AUTHORING Electron smoke —
// drives the REAL renderer UI (BIM mode panel) through a BrowserWindow.
//
// Launches a headless Xvfb display, then runs the Electron host in
// --smoke-bim mode against it. The 14 steps live in main.ts (runBimSmoke):
// mode toggle → create mini building → state count → move door opening
// +600 → cross-axis typed error → camera presets top/iso → build geometry
// through the OCCT worker (busy state; built/skipped counts) → occt:
// meshTokens → undo (meshToken gone) / redo (back) → save/open identical
// graph events hash → selection via the building tree → result file.
//
// Happy path expects the pinned toolchain (python3 + cadquery-ocp; set
// OFFISOS_OCCT_WORKER, optionally OFFISOS_PYTHON). Engine-free environments
// assert the typed engine_unavailable path instead (steps 8-11 adapt).
//
// Reproduce: cd apps/electron && npm run smoke:bim
// Verbose:   OFFISOS_SMOKE_VERBOSE=1 npm run smoke:bim
...
const tmp = mkdtempSync(join(tmpdir(), "offisos-electron-bim-smoke-"));
...
xvfb.on("error", (e) => {
  console.error("bim smoke: failed to spawn Xvfb:", e.message);
...
// 2. Start Electron in --smoke-bim mode against the Xvfb display.
const electronArgs = [APP, "--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage", "--smoke-bim"];
const child = spawn(electronExe, electronArgs, { cwd: APP, env, stdio: ["ignore", "pipe", "pipe"] });
attachIO(child);

// The build step realizes 5 solids through the OCCT worker (one disposable
// python per prepare) — the default budget reflects that.
const timeoutMs = Number(process.env.OFFISOS_SMOKE_TIMEOUT_MS || 300000);
const timer = setTimeout(() => {
  console.error(`bim smoke: TIMEOUT after ${timeoutMs}ms`);
  try {
    child.kill("SIGKILL");
  } catch {
    // ignore
...
child.on("exit", (code, signal) => {
  ...
  // inspectable BIM authoring smoke evidence.
  if (result) {
    try {
      mkdirSync(join(APP, "dist"), { recursive: true });
      writeFileSync(join(APP, "dist", "smoke-bim-result.json"), JSON.stringify(result, null, 2) + "\n");
    } catch {
      // ignore — not fatal
    }
  }
...
function printResult(result, out, err, status) {
  console.log("=== Electron 3D/BIM AUTHORING smoke result (real UI, OCCT when present) ===");
