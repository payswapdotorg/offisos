...
// --smoke-drafting mode against it. Engine-free (drafting never touches an
...
//     -> CADDocument command model -> drafting.* commands + drafting.snap
//       cause -> quantity.recalculate.requested -> quantity.changed ->
//     -> layers/entities/dimensions/trim/extend/undo/redo -> save/open
//     persistence with identical graph events hash.
//
// Reproduce: cd apps/electron && npm run smoke:drafting
// Verbose:   OFFISOS_SMOKE_VERBOSE=1 npm run smoke:drafting
...
const tmp = mkdtempSync(join(tmpdir(), "offisos-electron-drafting-smoke-"));
...
xvfb.on("error", (e) => {
  console.error("drafting smoke: failed to spawn Xvfb:", e.message);
...
// 2. Start Electron in --smoke-drafting mode against the Xvfb display.
const electronArgs = [APP, "--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage", "--smoke-drafting"];
...
const timer = setTimeout(() => {
  console.error(`drafting smoke: TIMEOUT after ${timeoutMs}ms`);
  try {
    child.kill("SIGKILL");
  } catch {
    // ignore
...
child.on("exit", (code, signal) => {
  ...
  if (result) {
    try {
      ...
      writeFileSync(join(APP, "dist", "smoke-drafting-result.json"), JSON.stringify(result, null, 2) + "\n");
    } catch {
      // ignore — not fatal
    }
  }
...
function printResult(result, out, err, status) {
  console.log("=== Electron 2D DRAFTING WORKFLOW smoke result (engine-free) ===");
