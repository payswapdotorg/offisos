// RESEARCH-CAD-007 / Issue #32: reproducible DOWNSTREAM IMPACT CASCADE
// Electron smoke — with the engine-free REFERENCE adapter bound as the
// geometry engine (the second engine inside the Electron host behind the
// same frozen boundary, LOCK-003).
//
// Launches a headless Xvfb display, then runs the Electron host in
// --smoke-impact mode against it. Engine-free (the reference adapter is pure
// TypeScript), so it runs on any toolchain. Proves the FULL chain:
//   BrowserWindow -> window.cad.send (preload) -> ipcRenderer.invoke
//     -> ipcMain.handle -> ElectronHost + IpcTransport -> AppApiHandler
//     -> immutable ModelHistory -> impact.cascade (model.version.created
//       cause -> quantity.recalculate.requested -> quantity.changed ->
//       estimate.recalculated -> rfq.scope.impact.detected -> commercial
//       impact) -> save/open persistence -> identical cascade hash.
//
// Reproduce: cd apps/electron && npm run smoke:impact
// Verbose:   OFFISOS_SMOKE_VERBOSE=1 npm run smoke:impact
...
const tmp = mkdtempSync(join(tmpdir(), "offisos-electron-impact-smoke-"));
...
xvfb.on("error", (e) => {
  console.error("impact smoke: failed to spawn Xvfb:", e.message);
...
// 2. Start Electron in --smoke-model mode against the Xvfb display.
const electronArgs = [APP, "--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage", "--smoke-impact"];
...
const timer = setTimeout(() => {
  console.error(`impact smoke: TIMEOUT after ${timeoutMs}ms`);
  try {
    child.kill("SIGKILL");
  } catch {
    // ignore
...
child.on("exit", (code, signal) => {
  ...
  // inspectable model-revision smoke evidence.
  if (result) {
    try {
      mkdirSync(join(APP, "dist"), { recursive: true });
      writeFileSync(join(APP, "dist", "smoke-impact-result.json"), JSON.stringify(result, null, 2) + "\n");
    } catch {
      // ignore — not fatal
    }
  }
...
function printResult(result, out, err, status) {
  console.log("=== Electron DOWNSTREAM IMPACT CASCADE smoke result (reference engine) ===");
