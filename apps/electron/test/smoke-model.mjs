// CAD-IMPLEMENT-003 / Issue #28: reproducible MODEL REVISIONS + Construction
// Graph bridge Electron smoke.
//
// Launches a headless Xvfb display, then runs the Electron host in
// --smoke-model mode against it. Engine-free (provenance engine ids are plain
// data), so it runs on any toolchain. Proves the FULL chain:
//   BrowserWindow -> window.cad.send (preload) -> ipcRenderer.invoke
//     -> ipcMain.handle -> ElectronHost + IpcTransport -> AppApiHandler
//     -> CADDocument immutable ModelHistory -> model.getHistory /
//     model.getGraphEvents (Construction Graph bridge) / model.replay
//     -> save/open persistence -> revision continuation.
//
// Reproduce: cd apps/electron && npm run smoke:model
// Verbose:   OFFISOS_SMOKE_VERBOSE=1 npm run smoke:model
...
xvfb.on("error", (e) => {
  console.error("model smoke: failed to spawn Xvfb:", e.message);
...
const electronArgs = [APP, "--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage", "--smoke-model"];
...
const timer = setTimeout(() => {
  console.error(`model smoke: TIMEOUT after ${timeoutMs}ms`);
  try {
    child.kill("SIGKILL");
  } catch {
    // ignore
...
child.on("exit", (code, signal) => {
  ...
  if (result) {
    try {
      ...
      writeFileSync(join(APP, "dist", "smoke-model-result.json"), JSON.stringify(result, null, 2) + "\n");
    } catch {
      // ignore — not fatal
    }
  }
...
function printResult(result, out, err, status) {
  console.log("=== Electron MODEL REVISIONS + Graph bridge smoke result ===");
