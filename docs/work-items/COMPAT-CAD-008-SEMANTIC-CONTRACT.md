...
- This document is a durable semantic contract for CC008. Lifecycle authorization is governed by `governance/work-items/COMPAT-CAD-008.json` and `governance/workflow-states.json`.
...
Each materialized member must have deterministic ARRAY provenance/ownership linking it to the ARRAY operation and source occurrence, with deterministic member index/order metadata where the existing canonical model supports it. This provenance must not become a competing application-local authority.

Source/member deletion semantics must be deterministic and must not leave orphaned ARRAY-owned canonical entities. Undo and redo must restore the exact canonical states required by that policy.
...
7. Source deletion and individual member deletion/modification follow the explicit deterministic ownership policy and preserve revision/history integrity.
...
- canonical provenance relating the operation to each materialized source occurrence/member.
...
The remediation evidence must additionally cover ARRAY/member provenance, source deletion/orphan prevention, and exact undo/redo around source deletion.
