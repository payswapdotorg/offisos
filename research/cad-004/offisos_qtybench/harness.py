"""Evidence harness for the RESEARCH-CAD-004 benchmark.

Vendored and adapted from research/cad-003/offisos_ifcbench/harness.py
(conventions proven in RESEARCH-CAD-001/002/003) so this evidence package
is independently reproducible without runtime coupling between work items.
...
def environment_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        env["ifctester"] = getattr(ifctester, "__version__", "unknown")
    except Exception as exc:  # pragma: no cover
        env["ifctester"] = f"unavailable: {exc}"
    try:
        import resource
